              ...
              <tr key={row.name} style={{
                ...
                <td style={{ padding: '10px 12px' }}>
                  <span style={{ fontWeight: 700, color: '#f59e0b' }}>
                    {(row as MetaEntry).faction_elo?.toFixed(0) ?? '—'}
                  </span>
                  {(row as MetaEntry).faction_elo_provisional && (
                    <span style={{ fontSize: 10, color: '#475569', marginLeft: 4 }}>P</span>
                  )}
                </td>
                ...
                <td style={{ padding: '10px 12px' }}>
                  {row.size && <Badge label={row.size} color={SIZE_COLORS[row.size] ?? '#888'} />}
                </td>
                <td style={{ padding: '10px 12px' }}>
                  {row.play && <Badge label={row.play} color={PLAY_COLORS[row.play] ?? '#888'} />}
                </td>
                <td style={{ padding: '10px 12px' }}>
                  {row.tricksy && <Badge label={row.tricksy} color={TRICKSY_COLORS[row.tricksy] ?? '#888'} />}
                </td>
              </tr>
            ))}
          </tbody>
